...
async function main() {
  ...
  console.log("isSanctioned:", await c.isSanctioned(process.env.TEST_ADDRESS));

  const p = await c.getProfile(process.env.TEST_ADDRESS);
  console.log("getProfile (V1 compat): score=" + p[0] + " tier=" + p[3] + " sanctioned=" + p[5] + " exists=" + p[6] + " tags=" + p[7].length);

  const r = await c.getRiskProfile(process.env.TEST_ADDRESS);
