/**
 * FidesOrigin Mumbai 测试网智能合约交互示例 (JavaScript/TypeScript Ethers.js v6)
 * 
 * 使用方法:
 *   npm install ethers dotenv
 *   node examples/interact_mumbai.js
 * 
 * 环境变量 (添加到 .env 文件):
...
class FidesOriginMumbaiClient {
  ...
  constructor(privateKey = null, rpcUrl = null) {
    ...
    // 初始化 Provider
    this.rpcUrl = rpcUrl || MUMBAI_CONFIG.rpcUrl;
  ...
  async connect() {
    ...
    console.log(`✅ Connected to ${MUMBAI_CONFIG.name}`);
   ...
   * @returns {Promise<string>} MATIC 余额
   */
   ...
   * @param {string} amountMatic - MATIC 金额
   * @returns {Promise<string>} 交易哈希
   */
  async sendMatic(to, amountMatic) {
...
async function main() {
  ...
  console.log('🚀 FidesOrigin Mumbai JavaScript SDK Demo');
  console.log('='.repeat(70));

  try {
    // 初始化客户端
    const client = new FidesOriginMumbaiClient();
    ...
    const maticBalance = await client.getMaticBalance();
...
module.exports = { FidesOriginMumbaiClient, MUMBAI_CONFIG };
