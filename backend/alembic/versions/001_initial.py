...
revision = '001'
# [AUDIT FIX 2026-09-18 R3-M20] 原与 000000000000 同为 down_revision=None
# 双根 → alembic upgrade head 报 multiple heads。挂到 000 下形成单链。
...
def upgrade() -> None:
    ...
    # 创建 risk_level 枚举类型
    # [AUDIT FIX 2026-09-18 R3-M20] 原枚举值为小写，而 models.py 的
    # ENUM(RiskLevel) 使用成员名（大写 LOW/MEDIUM/...）→ init_db 产物不一致。
    # [AUDIT FIX 2026-09-18 R3-M20] 与 models.py 对齐：枚举值为成员名大写、
    # 类型名 risk_level、含 UNKNOWN（models 默认值）。
    risk_level_enum = sa.Enum('LOW', 'MEDIUM', 'HIGH', 'CRITICAL', 'UNKNOWN', name='risk_level')
    risk_level_enum.create(op.get_bind())
    
    # [R3-M20] 原创建的 riskstatus 枚举无任何表列消费（models 中 status 均为
    # String 列）→ 死类型，不再创建。新增 event_status 枚举（risk_events.status 需要）。
    event_status_enum = sa.Enum('PENDING', 'CONFIRMED', 'FALSE_POSITIVE', 'UNDER_REVIEW', name='event_status')
    ...
    op.create_table('address_risks',
        ...
        sa.Column('risk_level', risk_level_enum, nullable=False, server_default='UNKNOWN'),  # [R3-M20] 与 models ENUM 对齐
    ...
    op.create_table('risk_events',
        ...
        sa.Column('status', event_status_enum, server_default='PENDING', nullable=True),  # [R3-M20] 与 models ENUM 对齐
